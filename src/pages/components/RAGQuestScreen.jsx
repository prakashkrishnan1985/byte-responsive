  ...
  const RAGQuestScreen = ({ leadInfo, onComplete }) => {
    ...
    const changePhase = (newPhase, reason = "") => {
      ...
      if (newPhase === PHASES.TYPING) {
        typingTimeoutRef.current = setTimeout(() => {
          setCurrentPhase((p) =>
            p === PHASES.TYPING ? PHASES.READY_TO_LISTEN : p
          );
        }, 3000);
      }
    };
    ...
    useEffect(() => {
      const handleAudioEnded = () => {
        ...
        if (currentPhase === PHASES.SPEAKING) {
          changePhase(
            PHASES.READY_TO_LISTEN,
            "audio ended  waiting for push-to-talk"
          );
        }
        //}
      };
    ...
    const getCurrentStep = async (sid = sessionId) => {
      ...
      try {
        const response = await axios.get(
          `${QUEST_API_BASE_URL}/quest/${sid}/step`
        );
    ...
    const submitResponse = async (input) => {
      ...
      try {
        ...
        const response = await axios.post(
          `${QUEST_API_BASE_URL}/quest/${sessionId}/respond`,
          {
    ...
    const handleChoiceSelect = (choice) => {
      if (
        currentPhase !== PHASES.READY_TO_LISTEN &&
        currentPhase !== PHASES.LISTENING
      )
        return;
    ...
    const handleTranscript = (text, isFinal = false) => {
      ...
      console.log(
        `🎤 Transcript received: "${text}", isFinal: ${isFinal}, isManualListening: ${isManualListening}`
      );
      ...
      return (
        <div className="rag-quest-layout">
          <div className="quest-error">
            <h2>Quest Error</h2>
            <p>{error}</p>
            <button onClick={startQuest} className="retry-button">
              Retry
            </button>
            <button
              onClick={handleComplete}
                        ...
                        if (currentPhase === PHASES.TYPING) {
                          ...
                          if (!currentStep.audio_path) {
                            ...
                            if (isSpeechMode && currentStep?.cta?.voice_allowed) {
                              changePhase(
                                PHASES.READY_TO_LISTEN,
                                "typing done, no audio, speech mode"
                              );
                            } else {
                              changePhase(
                                PHASES.READY_TO_LISTEN,
                                "typing done, no audio, text mode"
                              );
                            }
                          }
                        ...
                        <div className="speech-input-container">
                          ...
                          <div className="transcript-display">
                            {isSpeaking ? (
                              ...
                              <p>
                                {currentTranscript ||
                                  "Click the button above to speak"}
                              </p>
                            )}
                          </div>
        ...
        {process.env.NODE_ENV === "development" && (
          ...
          >
            ...
            <div style={{ marginTop: "8px", display: "flex", gap: "4px" }}>
              <button
                onClick={() =>
                  changePhase(PHASES.READY_TO_LISTEN, "manual debug")
                }
                style={{
                  fontSize: "10px",
                  padding: "2px 6px",
